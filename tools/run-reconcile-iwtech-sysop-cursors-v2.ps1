...
$LogFile = Join-Path $LogDir "reconcile-iwtech-sysop-cursors-v2-$Timestamp.log"
...
try {
    ...
    $Output = & $PythonExe -m scripts.reconcile_iwtech_sysop_sync_cursors_v2 2>&1
    ...
    if ($ExitCode -ne 0) {
        throw "IWTECH_SYSOP cursor reconciliation failed with exit code $ExitCode. See $LogFile"
    }
