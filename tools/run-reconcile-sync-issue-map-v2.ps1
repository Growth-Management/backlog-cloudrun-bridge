...
$LogFile = Join-Path $LogDir "reconcile-sync-issue-map-v2-$Timestamp.log"
...
try {
    ...
    $Output = & $PythonExe -m scripts.reconcile_sync_issue_map_v2 2>&1
    ...
    if ($ExitCode -ne 0) {
        throw "sync_issue_map reconciliation failed with exit code $ExitCode. See $LogFile"
    }
