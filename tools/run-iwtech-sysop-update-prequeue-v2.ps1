...
$LogFile = Join-Path $LogDir "iwtech-sysop-update-prequeue-v2-$Timestamp.log"
...
try {
    ...
    $Output = & $PythonExe -m scripts.prepare_iwtech_sysop_update_queue_v2 2>&1
    ...
    if ($ExitCode -ne 0) {
        throw "IWTECH_SYSOP update prequeue failed with exit code $ExitCode. See $LogFile"
    }
