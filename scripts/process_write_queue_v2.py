...
def settings() -> Settings:
    ...
    if google_auth_mode == "service_account":
        google_credentials = require_env("GOOGLE_APPLICATION_CREDENTIALS")
    elif google_auth_mode == "user_oauth":
        google_oauth_client_secret_file = require_env("GOOGLE_OAUTH_CLIENT_SECRET_FILE")
    else:
        raise RuntimeError("GOOGLE_AUTH_MODE must be either service_account or user_oauth")
...
def read_rows(service, s: Settings) -> list[tuple[int, dict[str, str]]]:
    ...
    if values:
        actual_headers = [str(value).strip() for value in values[0][:len(HEADERS)]]
        if actual_headers != HEADERS:
            raise RuntimeError("write_queue_v2 headers do not match expected A:AA schema")
    rows = []
...
def read_status_mapping(service, s: Settings) -> dict[tuple[str, str], str]:
    ...
    expected_headers = [
        "project_key", "status_name", "status_id", "is_active",
        "note", "last_checked_at", "source", "sort_order",
    ]
    headers = [str(value).strip() for value in values[0][:len(expected_headers)]]
    if headers != expected_headers:
        raise RuntimeError(f"{s.status_mapping_sheet_name} headers do not match expected mapping schema")
    ...
    for row_no, raw in enumerate(values[1:], start=2):
        ...
        if not project_key or not status_name or not status_id:
            raise RuntimeError(f"{s.status_mapping_sheet_name} row {row_no} has active mapping without required values")

        key = (project_key, status_name)
        if key in mapping:
            raise RuntimeError(f"{s.status_mapping_sheet_name} has duplicate active mapping for {project_key}/{status_name}")

        mapping[key] = status_id
...
def validate_required_queue_fields(row: dict[str, str]) -> None:
    if not row.get("queue_id", "").strip():
        raise ValueError("queue_id is required")
    if not row.get("project_key", "").strip():
        raise ValueError("project_key is required")
    if not row.get("idempotency_key", "").strip():
        raise ValueError("idempotency_key is required")

def find_duplicate_idempotency(
    rows: list[tuple[int, dict[str, str]]],
    current_row_no: int,
    row: dict[str, str],
) -> tuple[int, str] | None:
    idempotency_key = row.get("idempotency_key", "").strip()
    if not idempotency_key:
        return None

    blocking_statuses = {"queued", "processing", "validated", "succeeded"}
    for other_row_no, other in rows:
        if other_row_no == current_row_no:
            continue
        if other.get("idempotency_key", "").strip() != idempotency_key:
            continue
        other_status = other.get("status", "").strip().lower()
        if other_status in blocking_statuses and other_row_no < current_row_no:
            return other_row_no, other_status

    return None

def validate_due_date(value: str) -> str:
    due_date = value.strip()
    if not due_date:
...
def parse_payload(row: dict[str, str]) -> dict[str, str]:
    validate_required_queue_fields(row)
    ...
    if operation_type == "change_due_date":
        due_date = validate_due_date(str(payload.get("due_date") or row.get("due_date", "")))
...
def main() -> None:
    ...
    for row_no, row in rows:
        ...
        duplicate = find_duplicate_idempotency(rows, row_no, row)
        if duplicate:
            duplicate_row_no, duplicate_status = duplicate
            update_cells(service, s, row_no, [
                "on_hold", str(retry), "DUPLICATE_IDEMPOTENCY_KEY",
                f"idempotency_key already exists at row {duplicate_row_no} with status={duplicate_status}",
                "", "", s.worker_name,
            ])
            processed += 1
            continue
        ...
        try:
            ...
            if s.dry_run:
                if payload["action"] == "change_status":
                    ...
                    update_cells(service, s, row_no, [
                        "validated", str(retry), "", "",
                        f"dry_run: {payload['action']} validated", now_iso(), s.worker_name,
                    ])
                ...
                if not status_id:
                    ...
                    change_status(s, payload["target_issue_key"], status_id)
                    update_cells(service, s, row_no, [
                        "succeeded", str(retry), "", "",
                ...
                if not priority_id:
                    ...
                    change_priority(s, payload["target_issue_key"], priority_id)
                    update_cells(service, s, row_no, [
                        "succeeded", str(retry), "", "",
                ...
                if not assignee_id:
                    ...
                    change_assignee(s, payload["target_issue_key"], assignee_id)
                    update_cells(service, s, row_no, [
                        "succeeded", str(retry), "", "",
                ...
                change_due_date(s, payload["target_issue_key"], payload["due_date"])
                update_cells(service, s, row_no, [
                    "succeeded", str(retry), "", "",
